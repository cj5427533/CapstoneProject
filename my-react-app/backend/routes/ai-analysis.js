...
다음 형식으로 응답해주세요:
주의도 점수: 0-100 (숫자만)
주의 등급: LOW/MEDIUM/HIGH/CRITICAL
주요 우려사항: 3가지
권장사항: 3가지
`;
    ...
    const riskScore = parseInt(lines.find(line => line.includes('주의도 점수'))?.match(/\d+/)?.[0] || '0');
    const riskLevel = lines.find(line => line.includes('주의 등급'))?.split(':')[1]?.trim() || 'LOW';
    ...
    res.status(500).json({ 
      error: `쇼핑몰 주의도 분석 중 오류가 발생했습니다. ${error.message}` 
    });
  }
});
